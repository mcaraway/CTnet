class ChartsController < ApplicationController
  ...
  def items_by_week   
    ...
    render json: combine(Order.items_shipped_by_date(Date.today - 180,Date.today+1)
      .map { |order|
  ...
  def orders_by_week
    orders = Order.orders_by_week(Date.today - 180,Date.today+1).inject(Hash.new(0)){|h,k| k.week; h[k.week] += k.quantity;h}
